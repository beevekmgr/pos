...
class Cards extends Component {
	...
	handleChange = (e) => {
		...
		data.total = (this.props.tableItem.reduce((red,i)=>
	...
	componentDidMount(){
		let {data} =  this.state;
		data.returnAmount = - (this.props.tableItem.reduce((red,i)=>
		(red+ (((i.quantity)*(i.sellingPrice))-((i.discountPercent/100)*(i.quantity)*(i.sellingPrice)))),0).toFixed(2))
		this.setState({data});
	}
		...
		return (
			<div>
				...
				<div className="card return">
					...
					<div className="amount">{(this.state.data.returnAmount).toFixed(2)}</div>
				</div>
			</div>
		);
	}
