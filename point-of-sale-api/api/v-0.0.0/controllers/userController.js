...
const userController = {
  async login(req, res) {
    ...
    if (!doesUserExists) {
      res.json({
        success: false,
        errors: { mainError: "'user/password error' " }
      });
      return;
    }
    User.findOne(data, ("name", "username"), (err, user) => {
      if (err) {
        res.json({
          success: false,
          errors: { mainError: "'Error in sever' " }
        });
      } else {
        if (user) {
  ...
  async create(req, res) {
    ...
    if (doesUserExists) {
      res.json({
        success: false,
        errors: { mainError: "'User already exists' " }
      });
      return;
    }
    ...
    user.save((err, data) => {
      if (err) {
        ...
        res.json({
          success: false,
          errors: { mainError: "'Error in server' " }
        });
      } else {
